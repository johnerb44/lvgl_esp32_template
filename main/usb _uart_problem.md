...
copilot --resume=84463eef-bc35-47b5-a18f-316788335761

See: https://www.waveshare.com/wiki/ESP32-S3-Touch-LCD-4.3

## Development Environment

This program is the GUI application for a secure lockbox system running on an ESP32-S3 with an 800×480 Waveshare ESP32-S3-Touch-LCD-4.3 module ( https://www.waveshare.com/wiki/ESP32-S3-Touch-LCD-4.3 ), developed in the C programming language using ESP-IDF 5.1.6 and LVGL 9.3.0. 

The Waveshare ESP32-S3-Touch-LCD-4.3 module is a 4.3 inch touch LCD that includes an ESP32-S3 mpu, an SD card, I2C and SPI interfaces as well as several other interfaces. User records are stored on an SD card in a JSON file that conforms to the provided JSON Schema and are manipulated using cJSON. This GUI application is built around the LVGL library, v9.3. The IDE used for coding is vscode.

## Problem Statement
After program build/compile, program will not flash. Error says it can't connect to COM3. Can get it to flash only after power reset (unplug usb-c connector and plug it back in). This is also a problem using the ESP-IDF Monitor tool.  Monitor will run only once. Need to unplug/plug in USB to get Monitor to connect COM3 and run one more time.

The application seems to be changing USB-SEL on the CH422G I2C GPIO Expander Driver switching away from the USB-C UART connector.

In an effort to get the LCD display (mostly Backlight) and SD card working at the same time, troubleshooting was performed and changes to the Waveshare CH422G I2C GPIO Expander Driver were made and the SD Card and LCD Display both work correctly, but the USB-C UART is now broken without a power off reset.

I have included what was done to fix the SD/LCD problem, but broke the USB-C UART connection in the "Failed Solution Discussion" section of this document for reference.

## Context
SD card and LCD and USB-SEL use CH422G I2C expander...

USB_SEL is used to route the ESP32-S3’s USB data pair either to the on‑board USB‑to‑UART/debug connector or out to the CAN/“USB” header, by controlling the FSUSB42UMX analog USB switch.

What USB_SEL actually does
FSUSB42UMX is a dual‑DPDT high‑speed analog switch intended for USB2.0 D+/D− multiplexing.
​

On this board, its A/B ports are wired so one position connects the ESP32-S3’s internal USB D+/D− to the main USB Type‑C connector (used for power/flashing via CH343P), and the other position connects them to the CAN header, which is shared as an alternate USB interface.

The CH422G IO expander’s EXIO5 pin drives the FSUSB42UMX’s select pin (USB_SEL), so firmware can choose which physical port the ESP32-S3 USB device appears on, without needing extra ESP32 GPIOs.

In other words, the purpose of USB_SEL is to let software choose whether the ESP32-S3’s native USB goes to the main USB-C connector (default for flashing/debug) or to the alternate “CAN/USB” connector, and an incorrect or floating state on this line would absolutely present as intermittent or missing USB on the port.

## Status
SD/LCD solution Accepted - 2026-02-01

Waveshare module USB-C UART port is broken and only works (one time) after a power reset (unplug USB-C cable andplug it back in) of the USB.

## Solution for SD/LCD and Beginning of UART Problem Discussion
 ...
 * 
 * Problem solved: SD card SDCS (EXIO4) and LCD pins (EXIO1-3 )and USB-SEL(5)
...
The GPIO work is complete and functioning correctly. 

The USB issue is orthogonal to the CH422G implementation.
