# Fingerprint Authentication Flow Testing

## Feature: Secure lockbox fingerprint biometric authentication
...
### Scenario: Fingerprint Scan - Finger Found in R503 database and Finger Match found in user database
...
### Scenario: Fingerprint is found in the R503 database but does not match any registered user
    ...
    - And the R503 returned Step 3 packet contains ModelID
    - Then the R503 LED lights up Green
    - Then the system searches for a match of user's "FINGERID" field to R503 returned "ModelID"
    - When the system fails to find a match of user's "FINGERID" field to R503 returned "ModelID" 
    - Then the system sends the R503 an Aural command R503 to flash the LED Purple three times 
    - Then the system displays message "Fingerprint found - User match not found"
    - Suggest user re-enroll fingerprint here????
	- Then system stays on Fingerprint Scan screen
    - When the user selects "Back" button
    - Then the system returns to the biometric selection screen
   

### Scenario: Fingerprint is not found in the R503 database and finger has been placed on Fingerprint scanner
    - When the user selects "Begin authentication"
	- Then the system goes to Biometric screen
    - When the user selects "Fingerprint Scan"
	- Then the system goes to the Fingerprint Scan screen
    ...
    - Then the fingerprint scanner LED blinks red 3 times
    - Then the system displays message "Fingerprint not found - try another finger"
	- Then system stays on Fingerprint Scan screen
    - When the user selects "Back" button
    - Then the system returns to the biometric selection screen
	

### Scenario: Fingerprint is not found in the R503 database and finger has not been placed on R503          Fingerprint scanner (error code 02h: no finger on the sensor)
    - When the user selects "Begin authentication"
	- Then the system goes to Biometric screen
    - When the user selects "Fingerprint Scan"
	- Then the system goes to the Fingerprint Scan screen
    - When the user selects "Begin Scan"
    - And the user does not place a finger on the fingerprint scanner
    - Then the fingerprint scanner LED blinks blue 3 times
    - Then the fingerprint scanner LED blinks yellow 3 times
    - Then the system displays message "Finger not detected - place finger on scanner"
	- Then system stays on Fingerprint Scan screen
    - When the user selects "Back" button
    - Then the system returns to the biometric selection screen
...
### Scenario: Face Scan - Face found in hlk-tx510 database  - Face ID found in user database - PIN Match
...
- expected: goto: ui_screen_home on successful PIN match
 ...
 ### Scenario: Face is found in the hlk-tx510 database and matches a registered user
    - When the user selects "Begin authentication"
	- Then the system goes to Biometric screen
	...
	- And the system displays "Face found" for 3 seconds
	- Then the system proceeds to the PIN entry screen
	- STOP

### Scenario: Face Scan - Face Found in hlk-tx510 database -  Face ID found in user database - No PIN Match

- status: not tested


### Scenario: Face Scan - Face Not Found in hlk-tx510 database
